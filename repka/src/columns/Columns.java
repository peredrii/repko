...
public class Columns extends Applet implements Runnable {
	static final int SL = 12, Depth = 15, Width = 17, MaxLevel = 7,
	...
	void DrawFigure(Figure f) {
		DrawBox(f.x, f.y, f.c[0]);
		DrawBox(f.x, f.y + 1, f.c[1]);
		DrawBox(f.x, f.y + 2, f.c[2]);
	}
	...
	void PasteFigure(Figure f) {
		Fnew[f.x][f.y] = f.c[0];
		Fnew[f.x][f.y + 1] = f.c[1];
		Fnew[f.x][f.y + 2] = f.c[2];
	}
	...
	public void run() {
		...
		do {
			...
			while ((Fig.y < Depth - 2) && (Fnew[Fig.x][Fig.y + 3] == 0)) {
				if ((int) (System.currentTimeMillis() - tc) > (MaxLevel - Level)
				...
				} while ((int) (System.currentTimeMillis() - tc) <= (MaxLevel - Level)
						* TimeShift + MinTimeShift);
			}
